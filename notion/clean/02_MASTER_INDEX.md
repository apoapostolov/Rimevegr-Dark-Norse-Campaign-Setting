...
The franchise bible for the Iron Ledger project — a complete index of every data file, schema, script, test, and relationship in the Rimevegr simulation and novel-writing system.
...
All numbered Markdown files live at the project root. They serve as the world database for both novel-writing and roleplaying modes.
...
All data files live under `data/`. YAML is the canonical format. CJK-encoded text files contain spoiler content decoded via `scripts/spoiler_codec.py`.
...
All scripts live in `scripts/`. Python 3.10+. Run with `--help` for usage.
...
Events, contracts, and NPCs reference locations by settlement ID (matching `settlements.yaml` and `data/geography/atlas.yaml`). Bestiary entries feed into `encounter.py` and `barrow_generator.py`. Weather history drives `foraging.py` yields and `travel.py` hazards. Political state evolves via `village_politics.py` tick cycles.

---

## Cross-Reference Map

Which scripts read which data files, and which data files reference each other.
...
Volume outlines are in `data/volumes/`. Chapter templates are in `data/volumes/chapter_templates.yaml` (17 reusable types: battle, march, negotiation, barrow delve, aftermath, etc.).
